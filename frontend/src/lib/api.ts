...
export const whatsappAPI = {
    ...
    // New improved QR flow
    generateQRForNewDevice: async (data: { deviceName: string; description?: string }): Promise<{
        qrCode: string;
        expiresAt: string;
        sessionId: string;
    }> => {
        try {
            const response = await apiClient.post("/whatsapp/qr/generate", data);
            return response.data;
        } catch (error) {
            handleApiError(error);
            throw error;
    ...
    checkQRConnectionStatus: async (sessionId: string): Promise<{
        connected: boolean;
        deviceId?: string;
    }> => {
        try {
            const response = await apiClient.get(`/whatsapp/qr/status/${sessionId}`);
            return response.data;
        } catch (error) {
            handleApiError(error);
            throw error;
